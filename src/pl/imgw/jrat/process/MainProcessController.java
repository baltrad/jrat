...
import pl.imgw.jrat.scansun.ScansunParsedParameters;
...
public class MainProcessController {
	...
	if (cmd.hasOption(I)) {
	    FileProcessController.setInputFilesAndFolders(files, folders, cmd.getOptionValues(I));
	}

	/* display SCANSUN help message */
	if (cmd.hasOption(SCANSUN_HELP)) {
	    ScansunParsedParameters.printHelp();
	    return true;
	}

	/* print SCANSUN results */
	if (cmd.hasOption(SCANSUN_RESULT)) {
	    return ScansunProcessController.processScansunResult(cmd);
	}
	...
	if (cmd.hasOption(PRINTIMAGE)) {
	    PrintingImageProcessController.printImage(files, output, cmd.getOptionValues(PRINTIMAGE));
	}
	...
	if (cmd.hasOption(SCANSUN)) {
	    proc = ScansunProcessController.setScansunProcessor(cmd);
	}
	...
	if (proc == null || !proc.isValid()) {
	    LogHandler.getLogs().displayMsg("No valid process has been set", WARNING);
	    return false;
	}
	...
	if (cmd.hasOption(WATCH)) {
	    ...
	    t.start();
	    if (t.isAlive()) {
		LogHandler.getLogs().displayMsg("Watching process started", NORMAL);
		return true;
	    }
	} else if (cmd.hasOption(SEQ)) {
	    /* Starting sequence mode */

	    SequentialProcess seq = new SequentialProcess(proc, folders, cmd.getOptionValue(SEQ));
