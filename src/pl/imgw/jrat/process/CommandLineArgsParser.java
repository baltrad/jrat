...
public class CommandLineArgsParser {
    ...
    private static final String CALID_RESULT_DETAIL_DESCR = "CALID detailed result, " + "for more help use --calid-help option";
    private static final String CALID_RESULT_GNUPLOT_DESCR = "CALID gnuplot result, " + "for more help use --calid-help option";
    ...
    private final static String SCANSUN_RESULT_GNUPLOT_DESCR = "creates ps plots with results";
	...
	Option input_file = OptionBuilder.withArgName(FILES_ARG).hasArgs().withDescription(INPUT_DESCR).create(I);

	// options.addOption(input_file);

	Option output_file = OptionBuilder.withArgName(FILE_ARG).hasArg().withDescription(OUTPUT_DESCR).create(O);
	...
	Option print_img = OptionBuilder.withLongOpt(PRINTIMAGE).withDescription(PRINT_IMAGE_DESCR).hasArgs().create();

	Option seq = OptionBuilder.withLongOpt(SEQ).hasArgs().withDescription(SEQ_DESCR).create();
	options.addOption(seq);

	Option calid = OptionBuilder.withLongOpt(CALID).hasOptionalArgs().withDescription(CALID_DESCR).create();
	options.addOption(calid);

	Option calidResults = OptionBuilder.withLongOpt(CALID_RESULT).hasOptionalArgs().withDescription(CALID_RESULT_DESCR).create();
	options.addOption(calidResults);

	Option calidList = OptionBuilder.withLongOpt(CALID_LIST).hasOptionalArgs().withDescription(CALID_LIST_DESCR).create();
	options.addOption(calidList);

	options.addOption(null, CALID_HELP, false, CALID_HELP_DESCR);

	calidResults = OptionBuilder.withDescription(CALID_RESULT_DETAIL_DESCR).hasArgs().withLongOpt(CALID_RESULT_DETAIL).create();
	options.addOption(calidResults);

	calidResults = OptionBuilder.withDescription(CALID_RESULT_GNUPLOT_DESCR).hasArgs().withLongOpt(CALID_RESULT_GNUPLOT).create();
	options.addOption(calidResults);

	Option calidOpt = OptionBuilder.withArgName(FILE_ARG).hasArg().withDescription(CALID_OPT_DESCR).withLongOpt(CALID_OPT).create();
	options.addOption(calidOpt);

	Option scansun = OptionBuilder.withLongOpt(SCANSUN).hasArgs().withDescription(SCANSUN_DESCR).create();
	options.addOption(scansun);

	Option scansunResults = OptionBuilder.withLongOpt(SCANSUN_RESULT).hasOptionalArgs().withDescription(SCANSUN_RESULT_DESCR).create();
	options.addOption(scansunResults);

	Option scansunOpt = OptionBuilder.withArgName(FILE_ARG).hasArg().withDescription(SCANSUN_OPT_DESCR).withLongOpt(SCANSUN_OPT).create();
	options.addOption(scansunOpt);

	Option scansunDRAO = OptionBuilder.withArgName(FILE_ARG).hasArg().withDescription(SCANSUN_DRAO_DESCR).withLongOpt(SCANSUN_DRAO).create();
	options.addOption(scansunDRAO);

	options.addOption(null, SCANSUN_HELP, false, SCANSUN_HELP_DESCR);

	scansunResults = OptionBuilder.withDescription(SCANSUN_RESULT_GNUPLOT_DESCR).hasOptionalArgs().withLongOpt(SCANSUN_RESULT_GNUPLOT).create();
	options.addOption(scansunResults);
