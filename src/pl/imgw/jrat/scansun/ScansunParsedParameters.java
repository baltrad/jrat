...
public class ScansunParsedParameters {
	...
	public boolean initialize(String[] par) {
			...
			for (int i = 0; i < par.length; i++) {
				...
				if (par[i].startsWith(HEIGHT_MIN)) {
					...
					LogHandler.getLogs().displayMsg(
							errorMsg + " (" + par[i] + ")", WARNING);
					return false;
				}
			}
		} catch (NumberFormatException e) {
			LogHandler.getLogs().displayMsg(
					errorMsg + " (" + e.getLocalizedMessage() + ")", WARNING);
			return false;
		...
		if (getHeightMin() < 0 || getHeightMin() > 100) {
			LogHandler.getLogs().displayMsg(
					errorMsg + " (" + HEIGHT_MIN + getHeightMin() + ")",
					WARNING);
			return false;
		}
		if (getRangeMin() < 0 || getRangeMin() > 250) {
			LogHandler.getLogs().displayMsg(
					errorMsg + " (" + RANGE_MIN + getRangeMin() + ")", WARNING);
			return false;
		}

		if (getThresholdFraction() < 0 || getThresholdFraction() > 1) {
			LogHandler.getLogs().displayMsg(
					errorMsg + " (" + THRESHOLD_FRACTION
							+ getThresholdFraction() + ")", WARNING);
			return false;
		}

		if (getAngleDifference() < 0 || getAngleDifference() > 5) {
			LogHandler.getLogs().displayMsg(
					errorMsg + " (" + ANGLE_DIFFERENCE + getAngleDifference()
							+ ")", WARNING);
			return false;
		}
		...
		if (getMeanPowerFactor() < 0 || getMeanPowerFactor() > 0.1) {
			LogHandler.getLogs().displayMsg(
					errorMsg + " (" + MEAN_POWER_WIDTH_FACTOR
							+ getMeanPowerFactor() + ")", WARNING);
			return false;
		}
