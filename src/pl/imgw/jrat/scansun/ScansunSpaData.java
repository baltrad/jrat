...
public class ScansunSpaData {
    ...
    public boolean isValid() {
        ...
        if ((year < -2000) || (year > 6000)) {
            log.printMsg("Year not valid", Log.TYPE_ERROR, Log.MODE_VERBOSE);
            return false;
        }
        if ((month < 1) || (month > 12)) {
            log.printMsg("Month not valid", Log.TYPE_ERROR, Log.MODE_VERBOSE);
            return false;
        }
        if ((day < 1) || (day > 31)) {
            log.printMsg("Day not valid", Log.TYPE_ERROR, Log.MODE_VERBOSE);
            return false;
        }
        if ((hour < 0) || (hour > 24)) {
            log.printMsg("Hour not valid", Log.TYPE_ERROR, Log.MODE_VERBOSE);
            return false;
        }
        if ((minute < 0) || (minute > 59)) {
            log.printMsg("Minute not valid", Log.TYPE_ERROR, Log.MODE_VERBOSE);
            return false;
        }
        if ((second < 0) || (second > 59)) {
            log.printMsg("Second not valid", Log.TYPE_ERROR, Log.MODE_VERBOSE);
            return false;
        }
        if (Math.abs(timezone) > 18.0) {
            log.printMsg("Timezone not valid", Log.TYPE_ERROR, Log.MODE_VERBOSE);
            return false;
        }

        if (Math.abs(longitude) > 180.0) {
            log.printMsg("Longitude not valid", Log.TYPE_ERROR,
                    Log.MODE_VERBOSE);
            return false;
        }
        if (Math.abs(latitude) > 90.0) {
            log.printMsg("Latitude not valid", Log.TYPE_ERROR, Log.MODE_VERBOSE);
            return false;
        }
        if (altitude < -6500000.0) {
            log.printMsg("Altitude not valid", Log.TYPE_ERROR, Log.MODE_VERBOSE);
            return false;
        }

        if ((pressure < 0.0) || (pressure > 5000.0)) {
            log.printMsg("Pressure not valid", Log.TYPE_ERROR, Log.MODE_VERBOSE);
            return false;
        }
        if ((temperature <= -273.0) || (temperature > 6000.0)) {
            log.printMsg("Temperature not valid", Log.TYPE_ERROR,
                    Log.MODE_VERBOSE);
            return false;
        }
        if (Math.abs(slope) > 360.0) {
            log.printMsg("Slope not valid", Log.TYPE_ERROR, Log.MODE_VERBOSE);
            return false;
        }
        if (Math.abs(atmosphericRefraction) > 5.0) {
            log.printMsg("Atmospheric refraction not valid", Log.TYPE_ERROR,
                    Log.MODE_VERBOSE);
            return false;
        }
        if (Math.abs(deltaT) > 8000.0) {
            log.printMsg("Delta_t not valid", Log.TYPE_ERROR, Log.MODE_VERBOSE);
            return false;
        }

        if (Math.abs(azimuthRotation) > 360.0) {
            log.printMsg("Azimuth rotation not valid", Log.TYPE_ERROR,
                    Log.MODE_VERBOSE);
            return false;
        }
