 ...
 * 
 */
public class ScansunSolarPositionAlgorithmSolver {
	...
	private static final int BAD_VALUE = -99999;

	public ScansunSolarPositionAlgorithmSolver() {
	}
	...
	public ScansunSolarPositionAlgorithmSolver(
			ScansunSolarPositionAlgorithmSolver source) {
		this.jd = source.jd;
		this.jc = source.jc;
		this.jde = source.jde;
		this.jce = source.jce;
		...
		this.params = new ScansunSolarPositionAlgorithmParameters(source.params);
	}

	public void setSolarPositionAlgorithmParameters(
			ScansunSolarPositionAlgorithmParameters params) {
		this.params = params;
	}
	...
	private double calculateJulianDay() {
		...
		int day = params.getDayOfMonth();
		int hour = params.getHourOfDay();
		int minute = params.getMinute();
		int second = params.getSecond();
		double timezone = params.getTimezone();

		double dayDecimal = day
				+ (hour - timezone + (minute + second / 60.0) / 60.0) / 24.0;
	...
	private double calculateJulianEphemerisDay(double jd) {
		return jd + params.getDeltaT() / 86400.0;
	}
	...
	private double calculateEarthPeriodicTermSummation(double jme,
			double[][] terms, int count) {
		int i;
		double sum = 0;
	...
	private double calculateEarthHeliocentricLongitude(double jme) {
		...
		for (i = 0; i < L_SUBCOUNT.length; i++)
			sum[i] = calculateEarthPeriodicTermSummation(jme, L_TERMS[i],
					L_SUBCOUNT[i]);

		return limitDegrees(Math.toDegrees(earthValues(jme, sum,
				L_SUBCOUNT.length)));
	}
	...
	private double calculateEarthHeliocentricLatitude(double jme) {
		...
		for (i = 0; i < B_SUBCOUNT.length; i++)
			sum[i] = calculateEarthPeriodicTermSummation(jme, B_TERMS[i],
					B_SUBCOUNT[i]);
	...
	private double calculateEarthRadiusVector(double jme) {
		...
		for (i = 0; i < R_SUBCOUNT.length; i++)
			sum[i] = calculateEarthPeriodicTermSummation(jme, R_TERMS[i],
					R_SUBCOUNT[i]);
	...
	private double calculateEclipticTrueObliquity(double deltaEpsilon,
			double epsilon0) {
	...
	private double calculateApparentSunLongitude(double theta, double deltaPsi,
			double deltaTau) {
	...
	private double calculateGreenwichSiderealTime(double nu0, double deltaPsi,
			double epsilon) {
		return nu0 + deltaPsi * Math.cos(Math.toRadians(epsilon));
	}

	private double calculateGeocentricSunRightAscension(double lamda,
			double epsilon, double beta) {
		double lamdaRad = Math.toRadians(lamda);
		double epsilonRad = Math.toRadians(epsilon);

		return limitDegrees(Math
	...
	private double calculateGeocentricSunDeclination(double lamda,
			double epsilon, double beta) {
	...
	private double calculateObserverHourAngle(double nu, double alpha) {
		return limitDegrees(nu + params.getLongitude() - alpha);
	}
	...
	private double calculateSunRightAscensionParallax(double xi, double h,
			double delta) {
		double deltaAlphaRad;
		double latitudeRad = Math.toRadians(params.getLatitude());
		...
		double x = Math.cos(u) + params.getAltitude() * Math.cos(latitudeRad)
				/ 6378140.0;
	...
	private double calculateTopocentricSunDeclination(double xi, double h,
			double delta) {
		double deltaAlphaRad;
		double latitudeRad = Math.toRadians(params.getLatitude());
		double xiRad = Math.toRadians(xi);
		...
		double deltaRad = Math.toRadians(delta);
		double u = Math.atan(0.99664719 * Math.tan(latitudeRad));
		double y = 0.99664719 * Math.sin(u) + params.getAltitude()
	...
	private double calculateTopocentricSunRightAscension(double alpha,
			double deltaAlpha) {
		return alpha + deltaAlpha;
	}

	private double calculateTopocentricLocalHourAngle(double h,
			double deltaAlpha) {
		return h - deltaAlpha;
	}

	private double calculateTopocentricElevationAngle(double deltaPrime,
			double hPrime) {
		double latitudeRad = Math.toRadians(params.getLatitude());
	...
	private double calculateAtmosphericRefractionCorrection(double e0) {
		if (e0 >= -1 * (SUN_RADIUS + params.getAtmosphericRefraction())) {
			return (params.getPressure() / 1010.0)
					* (283.0 / (273.0 + params.getTemperature()))
					* 1.02
	...
	private double calculateTopocentricElevationAngleCorrected(double e0,
			double deltaE) {
		return e0 + deltaE;
	}

	private double calculateTopocentricAzimuthAngleNeg180Pos180(double hPrime,
			double deltaPrime) {
	...
	private double calculateApproximateSunTransitTime(double alphaZero,
			double nu) {
		return (alphaZero - params.getLongitude() - nu) / 360.0;
	}
	...
	private double dayFractionToLocalHour(double dayfrac) {
		return 24.0 * limitZero2one(dayfrac + params.getTimezone() / 24.0);
	}
	...
	private double calculateSunRiseAndSet(double[] mRts, double[] hRts,
		...
		return mRts[sun]
				...
				/ (360.0 * Math.cos(Math.toRadians(dPrime[sun]))
						* Math.cos(Math.toRadians(params.getLatitude())) * Math
	...
	private double[] calculateEoTAndSTS() {
		...
		double h0prime = -1 * (SUN_RADIUS + params.getAtmosphericRefraction());

		ScansunSolarPositionAlgorithmSolver sunRtsSolver = new ScansunSolarPositionAlgorithmSolver(
				this);

		// double m = calculateSunMeanLongitude(jme);
		// double eot = calculateEoT(m, alpha, deltaPsi, epsilon);

		sunRtsSolver.params.setHourOfDay(0);
		sunRtsSolver.params.setMinute(0);
		sunRtsSolver.params.setSecond(0);
		sunRtsSolver.params.setTimezone(0.0);

		sunRtsSolver.jd = sunRtsSolver.calculateJulianDay();
		sunRtsSolver.calculateGeocentricSunRightAscensionAndDeclination();

		nu = sunRtsSolver.nu;

		sunRtsSolver.params.setDeltaT(0);
		sunRtsSolver.jd = jd--;

		for (int i = 0; i < JD.JD_COUNT.ordinal(); i++) {
			sunRtsSolver.calculateGeocentricSunRightAscensionAndDeclination();
			a[i] = sunRtsSolver.alpha;
			d[i] = sunRtsSolver.delta;
			sunRtsSolver.jd++;
		}

		mRts[SUN.SUN_TRANSIT.ordinal()] = calculateApproximateSunTransitTime(
				a[JD.JD_ZERO.ordinal()], nu);
		...
		if (h0 >= 0) {
			...
			for (int i = 0; i < SUN.SUN_COUNT.ordinal(); i++) {
				...
				n = mRts[i] + params.getDeltaT() / 86400.0;
				aPrime[i] = calculateRtsAlphaDeltaPrime(a, n);
				dPrime[i] = calculateRtsAlphaDeltaPrime(d, n);

				hPrime[i] = limitDegrees180pm(nuRts[i] + params.getLongitude()
						- aPrime[i]);
			...
			result[3] = dayFractionToLocalHour(mRts[SUN.SUN_TRANSIT.ordinal()]
					- hPrime[SUN.SUN_TRANSIT.ordinal()] / 360.0);// suntransit
			result[4] = dayFractionToLocalHour(calculateSunRiseAndSet(mRts,
					hRts, dPrime, hPrime, h0prime, SUN.SUN_RISE.ordinal()));// sunrise
			result[5] = dayFractionToLocalHour(calculateSunRiseAndSet(mRts,
					hRts, dPrime, hPrime, h0prime, SUN.SUN_SET.ordinal()));// sunset
		} else {
			result[0] = BAD_VALUE;// srha
			result[1] = BAD_VALUE;// ssha
			result[2] = BAD_VALUE;// sta

			result[3] = BAD_VALUE;// suntransit
			result[4] = BAD_VALUE;// sunrise
			result[5] = BAD_VALUE;// sunset
		}
	...
	private double calculateSurfaceIncidenceAngle(double zenith,
			double azimuth180) {
		double zenithRad = Math.toRadians(zenith);
		double slopeRad = Math.toRadians(params.getSlope());

		return Math.toDegrees(Math.acos(Math.cos(zenithRad)
				* Math.cos(slopeRad)
				+ Math.sin(slopeRad)
				* Math.sin(zenithRad)
				* Math.cos(Math.toRadians(azimuth180
						- params.getAzimuthRotation()))));
	}

	private double calculateEoT(double m, double alpha, double deltaPsi,
			double epsilon) {
		return limitMinutes(4.0 * (m - 0.0057183 - alpha + deltaPsi
				* Math.cos(Math.toRadians(epsilon))));
	}

	public double getElevation() {
		return calculateTopocentricElevationAngleCorrected(e0, deltaE);
	}

	public double getAzimuth() {
		double azimuth180 = calculateTopocentricAzimuthAngleNeg180Pos180(
				hPrime, deltaPrime);
		return calculateTopocentricAzimuthAngleZero360(azimuth180);
	}
